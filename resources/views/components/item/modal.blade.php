...
<div id="hs-vertically-centered-scrollable-modal"
     class="hs-overlay  transition-all duration-300 opacity-0 w-full h-full fixed top-0 left-0 z-[80] overflow-x-hidden overflow-y-auto justify-center items-center"
...
>
    ...
    >
        <div x-data="{formType: $wire.entangle('formType')}"
            ...
            <div  class="flex justify-between items-center py-3 px-4 border-b dark:border-olblue-800/[0.5]">
                <h3 class="flex justify-start items-center font-bold text-gray-800 dark:text-white">
                    @switch($type)
                        ...
                        @case('delete')
                        <x-svg.trash class="flex w-4 h-4"></x-svg.trash>
                        @break
                    @endswitch
                ...
                <x-item.button @click.prevent="isFirstModelButtonClicked = false" formType="default" size="small"
